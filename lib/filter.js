...
module.exports = function (data) {
  ...
  const filterATagHrefExternal = data => {
    return data.replace(/<a.*?(href=['"](.*?)['"]).*?>/gi, (aTagSrc, hrefAttrStr, href) => {
      ...
      aTagSrc = addAttr(aTagSrc, hrefAttrStr, 'referrerpolicy', config.nofollow.referrerpolicy);
      return addAttr(aTagSrc, hrefAttrStr, 'rel', config.nofollow.rel);
    });
  };
  ...
  const filterImgTagSrcExternal = data => {
    return data.replace(/<img.*?(src=['"](.*?)['"]).*?>/gi, (imgTagSrc, srcAttrStr, src) => {
      ...
      imgTagSrc = addAttr(imgTagSrc, srcAttrStr, 'referrerpolicy', config.nofollow.referrerpolicy);
      return addAttr(imgTagSrc, srcAttrStr, 'rel', config.nofollow.rel);
    });
  };
